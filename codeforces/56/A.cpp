...
int main() {
    ...
    alcohol.push_back("ABSINTH"); alcohol.push_back("BEER"); alcohol.push_back("BRANDY"); alcohol.push_back("CHAMPAGNE");
    alcohol.push_back("GIN"); alcohol.push_back("RUM"); alcohol.push_back("SAKE"); alcohol.push_back("TEQUILA");
    alcohol.push_back("VODKA"); alcohol.push_back("WHISKEY"); alcohol.push_back("WINE");
    int cnt = 0;
    for(int i = 0; i < n; i++) {
	string n; cin >> n;
	if(!isdigit(n[0])) {
	    if(find(be(alcohol), n) != alcohol.end())
		cnt++;
	} else cnt += (stoi(n) < 18);
