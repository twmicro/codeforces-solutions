...
int main() {
    ...
    cin >> n;
    vi a = read_vi(n);
    int cnt = 0, mode = 1, lst = 0, lstlst = 0, l = -1, r = -1;
    for(int i = 0; i < n; i++) {
	//cout << "mode=" << mode << endl;
	if(mode) {
	    if(a[i] - lst != 1) {
		//cout << "Got unexpected number! I: " << i + 1 << endl;
		if(cnt) {
		    //cout << "Second time! It's time to ban! I: " << i + 1 << endl;
		    l = r = -1;
		    break;
		}
		mode = !mode;
		...
		r = i + a[i] - lst - 1;
		lstlst = a[i];
		//cout << "Expecting l=" << l + 1 << ",r=" << r + 1 << endl;
	    }
	    lst = a[i];
	} else {
	    if(lst - a[i] != 1) {
		//cout << "Oops! Unexpected number, but already in decreasing order! I: " << i + 1 << endl;
		l = r = -1;
		break;
	    }
	    lst = a[i];
