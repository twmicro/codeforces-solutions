...
int main() {
    ...
    while(t--) {
        int n, d; cin >> n >> d;
        vi a = read_vi(n);
        int min1 = distance(a.begin(), min_element(be(a)));
        int min2 = INT32_MAX;
        bool isThereError = false;
        for(int i = 0; i < n; i++) {
            if(i != min1) min2 = min(min2, a[i]);
            if(a[i] > d) isThereError = true;
        }
        if(!isThereError) cout << "YES" << endl;
        else cout << ((a[min1] + min2 <= d) ? "YES" : "NO") << endl;
    }
	return 0;
}
