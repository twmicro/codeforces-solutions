...
vi prepare_countmap(string s) {
    vi cm;
    int segsize = 0;
    for(int i = 0; i < s.size(); i++) {
        if(s[i] == 'v') {
            segsize++;
            cm.add(i > 0 ? (segsize > 1 ? cm[i-1]+1 : cm[i-1]): 0);
        }
        else cm.add(i > 0 ? cm[i-1] : 0), segsize = 0;
    }
    cm.add(cm[cm.size()-1]);
    return cm;
}
...
int main() {
    ...
    string s;
    cin >> s;
    vi cm = prepare_countmap(s);
    ll cnt = 0;
    for(int i = 0; i < s.size(); i++) {
        if(s[i] == 'o') {
            ll left = cm[i];
            ll right = cm[cm.size() - 1] - cm[i];
            cnt += left * right;
        }
    }
    cout << cnt << endl;
